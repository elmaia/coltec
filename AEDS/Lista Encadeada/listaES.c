/**
 * @file   lista.c
 * @brief  Arquivo com a implementação do TAD Lista.
 ...
 */
static erro_t listaES_obtemElementoPosicao(listaES_t * lista, uint32_t posicao, itemLista_t ** item) {
...
erro_t listaES_removeElemento(listaES_t * lista, dadosItem info) {
  ...
  while (atual != NULL) {
    if (atual->info == info) break;
    anterior = atual;
    atual = atual->proximo;
  }

  // Verifica se o item foi encontrado, ou seja, se o atual não é nulo
  if (atual == NULL) { return ERRO_LISTA_ITEM_NAO_ENCONTRADO; }
...
erro_t listaES_removePosicao(listaES_t * lista, uint32_t posicao) {
  ...
  erro_t erro;
  itemLista_t * elementoARemover, * anterior;
  ...
  if (posicao == 0) {
    elementoARemover = (itemLista_t *)lista->primeiro;
...
bool_t listaES_busca(listaES_t * lista, dadosItem info, uint32_t * posicao) {
  ...
  uint32_t pos = 0;
  itemLista_t * proximo = (itemLista_t *)lista->primeiro;
