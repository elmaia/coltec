/**
* @file   Aula14.c
* @brief  Arquivo com a implementação dos exercícios da Aula 14 em binário.
...
int arquivo_obtemNumeroLinhas(FILE * arquivo) {
  ...
  char caractere;
  bool_t ultimoLidoQuebraLinha = FALSO;
  bool_t leuAlgumCaractere = FALSO;
  ...
  for (;;) {
    if (fread(&caractere, sizeof(char), 1, arquivo) != 1) {
      // Terminou de ler

      // Se o último caractere lido não é uma quebra de linha, então finalizou uma linha que não tinha \n
      if ((!ultimoLidoQuebraLinha) && (leuAlgumCaractere)) nLinhas++;
      break;
    }
    else {
      leuAlgumCaractere = VERDADEIRO;
      if (caractere == '\n') {
        nLinhas++;
        ultimoLidoQuebraLinha = VERDADEIRO;
      }
      else {
        ultimoLidoQuebraLinha = FALSO;
      }
    }
  }
...
int arquivo_obtemNumeroCaracteres(FILE * arquivo) {
  ...
  for (;;) {
    if (fread(&caractere, sizeof(char), 1, arquivo) != 1) {
      ...
      if ((caractere == '\n') || (caractere == '\r') || (caractere == ' ')) continue;
      nCaracteres++;
    }
  }
...
#define CAMINHO_ARQUIVO_ENTRADA_ALUNOS  "./dadosAlunosEntrada.bin"
...
void exercicio03e04(void) {
  ...
  arquivo = fopen(CAMINHO_ARQUIVO_ENTRADA_ALUNOS, "rb");
  if (arquivo == NULL) {
  ...
  if (fread(&nRegistros, sizeof(int), 1, arquivo) != 1) {
    // Falha ao ler o arquivo
    fprintf(stderr, "Falha ao ler o numero de registros do arquivo.\n");
    return;
  }
